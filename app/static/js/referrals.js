...
class ReferralSystem {
			...
			const marker = L.marker([hospital.lat, hospital.lng], { icon }).addTo(
				this.map
                    ...
                    <div class="hospital-popup">
                        ...
                        ${
													hospital.level
														? `<p>Level: <strong>${hospital.level}</strong></p>`
														: ''
												}
	...
	respondToReferral(responseType) {
		...
		const responseMessage = prompt(
			`Please provide a reason for ${responseType}ing this referral:`
		);
		...
		fetch('/referrals/api/respond-to-referral', {
			...
			.catch((error) => {
				console.error('Error:', error);
				showAlert(
					'An error occurred while responding to the referral',
					'error'
				);
			});
	}
		...
		detailsDiv.innerHTML = `
            <div class="row">
                ...
                <div class="col-md-6">
                    ...
                    <p><strong>Urgency:</strong> <span class="badge bg-${this.getUrgencyColor(
											referral.urgency
										)}">${referral.urgency}</span></p>
	...
	resetForm() {
		...
		document.getElementById('targetHospitalSelect').innerHTML =
			'<option value="">Select hospital from map...</option>';
	}
	...
	updateActiveReferralsList() {
		...
		this.activeReferrals.forEach((referral, id) => {
			...
			item.innerHTML = `
                ...
                <span class="badge bg-warning">${minutes}:${seconds
				.toString()
				.padStart(2, '0')}</span>
            `;
